...
function metaEvents(platform) {
	...
	platform.prototype.savePersistentDevice = function savePersistentDevice(device) {
		var persistentDevices = this.opts.persistentDevices || [ ];
		var guid = [ device.G, device.V, device.D ].join("_")
		if (persistentDevices.indexOf(guid) < 0) {
			persistentDevices.push(guid);
			this.opts.persistentDevices = persistentDevices;
			this.save();
		}
	}
	...
	platform.prototype.setArduinoVersion = function setArduinoVersion(v) {
		this.version = v;
		this.log.debug("arduino version: %s", v);
		this.emit('version', v);
		if ((v == null) || (v.arduinoModel == 'V12')) {
			var rfDevice = { G : "0"	
					...
					, D : 11
              		...
              		this.savePersistentDevice(rfDevice);
			var ninaEyes = { G : "0"	
					, V : 0
					, D : 1007
					};
                	this.savePersistentDevice(ninaEyes);
		}
	}
